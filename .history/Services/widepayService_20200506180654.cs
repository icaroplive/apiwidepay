    ...
    public class widepayService : IWidePay {
        ...
        public async Task<RetornoCancela> cancelarCobrancas (List<string> idwidepay) {
            ...
            var request = new HttpRequestMessage {
                RequestUri = new Uri (url),
                Method = HttpMethod.Post,
                Content = new StringContent (content.ToString (), Encoding.UTF8, ContentType.Json)
            };
           ...
           // var result = await _httpClient.PostAsJsonAsync (url, new CancelaModel () { id = idwidepay });
