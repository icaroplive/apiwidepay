...
namespace api_widepay.Services {
    ...
    public class widepayService : IWidePay {
        ...
        public async Task<RetornoCancela> cancelarCobrancas (List<string> idwidepay) {
            ...
            //var nvc = new List<KeyValuePair<string, string>> ();
            //nvc.Add (new KeyValuePair<string, string> ("id", String.Format("[{0}]",idwidepay.ToArray()) ));
           ...
           /* var request = new HttpRequestMessage {
            ...
            var result = await _httpClient.PostAsJsonAsync (url,new StringContent (content.ToString(), Encoding.UTF8, "application/json"));
