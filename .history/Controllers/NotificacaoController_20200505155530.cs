...
namespace api_widepay.Controllers {
    ...
    public class NotificacaoController : ControllerBase {
        ...
        public Cobrancas consultarcobrancas () {
            var not = _cob.consultarCobrancas ().Result;
            var dtinicio = Convert.ToDateTime("2020-04-01");
            not.cobrancas = not.cobrancas.Where( c => c.status == "Recebido").Where(c => c.vencimento >= dtinicio).ToList();
            not.total = not.cobrancas.Count();
            return not;
        }
