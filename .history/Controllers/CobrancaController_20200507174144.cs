...
namespace api_widepay.Controllers {
    public class CobrancaController : ControllerBase {
        ...
        [HttpPost]
        public List<RetornoBoleto> Post ([FromBody] List<int> idfin_movimento) {
            var lista = new List<RetornoBoleto>();
            foreach (var x in idfin_movimento) {
                var result = _cob.criarCobranca (x);
                var boleto = _cob.pegarCodigoBarra (result.Result.id);
                _mysql.atualizarFinMovimento (x, result.Result.id, boleto.Result.codigo);
                lista.Add(result.result);
            }
            return lista;
        }
